...
-   **Order Management**: View, track, and update order details, including shipping information which automatically updates Shopware. Displays dual status badges for order processing and payment status. Supports bulk tracking number updates for multiple orders simultaneously.
-   **Shipping Dashboard**: Permission-restricted view (viewShipping) showing orders ready for shipping (Paid OR Authorized payment status AND In Progress order status). Automatically detects equipment requirements (Mitnahmestapler/Hebebühne) from order line items and custom fields, displaying equipment badges for quick identification.
-   **Ticketing System**: Fully integrated ticketing system with permission-based access (viewTickets, manageTickets). Features include ticket creation (with optional order linking), status and priority management, user assignment with assignee filtering, "My Tickets" personal view for assigned tickets, internal/public comments, comprehensive filtering (status, priority, category, assignee, tags), search, pagination, email import (.msg/.eml with PDF/photo filtering), activity logging, due dates with SLA warnings, CSV/Excel export with comprehensive filtering, automatic ticket assignment (round-robin and rule-based), response templates with CRUD management, and AI-powered text improvement via OpenAI integration. Tickets can be created directly from OrderDetailModal or from email drag & drop. Auto-assignment can be configured via the Ticket Rules settings page (accessible to manageTickets users) with priority-based rule evaluation and round-robin fallback. **Note**: Email notifications require external email service setup (Resend/SendGrid integration pending - credentials: EMAIL_API_KEY, EMAIL_FROM_ADDRESS).
-   **Ticket Templates**: Template management system (manageTickets permission) for creating, editing, and deleting reusable ticket response templates. Templates can be inserted into ticket comments via dropdown selector in both CreateTicketDialog and TicketDetailModal.
-   **AI Text Improvement**: OpenAI-powered text enhancement for ticket responses. Settings page (manageSettings permission) allows configuring OpenAI API key (stored encrypted with AES-256-GCM) and enabling/disabling the feature. When enabled, users can improve their ticket comment text using the Sparkles button, which processes text through GPT-4o-mini with German customer service optimization prompts.
...
-   **Internationalization**: Full i18n support across the application for order statuses, filter panels, analytics, ticketing system, shipping dashboard, bulk actions, templates, AI features, and general UI elements in German, English, and Spanish.
...
-   **OpenAI SDK**: AI-powered text improvement using GPT-4o-mini model for ticket response enhancement.